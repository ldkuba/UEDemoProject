...
#include "LevelEditorActions.h"  // action for building reflections only

#include "Networking.h"
#include "Engine.h"  // has GEngine with a bunch of debug code like screen messages
...
void SubscribeToVRKeyEvents(UWorld* InWorld)
{
	...
	InWorld->GetTimerManager().SetTimer(TimerHandle, [InWorld, TimerHandle]()
		{
...
{	
	...
	for (auto& CommandStr : MetaCommandList)
	{
		...
		else if (CommandName == "get_vertex_data_from_scene_objects" && CommandParameterList.Num() > 0)
		{
			...
			TSharedPtr<FJsonObject> RootObject = MakeShareable(new FJsonObject);  // blank root json object
...
FSocket* CreateTCPListenerSocket(const FString& YourChosenSocketName, const FString& TheIP, const int32 ThePort, const int32 ReceiveBufferSize)
{
	...
	if (!ListenSocket) {		
		...
		return false;  // sometimes the socket doesn't get created correctly - FTcpSocketBuilder: Failed to create the socket SingleMessageSocket as configured
	}
	//Set Buffer Size
	int32 NewSize = 0;
